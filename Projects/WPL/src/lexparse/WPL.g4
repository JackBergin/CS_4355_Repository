...
scalarDeclaration: (t = type | VAR) scalars += scalar (
		',' scalars += scalar
	)* ';';

scalar: id = ID varInit = varInitializer?;

arrayDeclaration: typename = type '[' INTEGER ']' ID ';';
...
externDeclaration:
	'extern' (externProcHeader | externFuncHeader) ';';

procedure: procH = procHeader blc = block;
...
function: funcH = funcHeader blc = block;
...
params: (
		types += type ids += expr (',' types += type ids += expr)*
	);
...
loop: 'while' e = expr 'do' blc = block;

conditional:
	'if' e = expr 'then'? validBlock = block (
		'else' invalidBlock = block
	)?;
...
assignment:
	targets += ID (',' targets += ID)* '<-' exprs += expr (
		',' exprs += expr
	)* ';'
	| arrayIndex '<-' e += expr ';';
